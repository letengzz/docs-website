...
| 81 | Frontend | 数据可视化 ✅ | 概述与选型、渲染路线（Canvas/SVG/WebGL）、ECharts 深入、数据到图形的映射、大数据量性能、数据大屏工程、实战（**库内空白**：全库「数据可视化」仅 1 处提及、「数据大屏」0 命中、无独立主题；本日新建 `docs/Frontend/DataVisualization` 9 页） |
| 82 | Frontend | 状态管理 ✅（**改道**：转既有页深化） | 原定「Pinia、Redux/Zustand、对比选型」与库内既有专题重叠——已有 `Vue3/Pinia`、`Vue3/PiniaAdvanced` 以及 React / Angular / Next / UmiJS / Uniapp 各自的 `StateManagement`，共 8 个页面；按 AGENTS.md 第 5 节「已有内容的主题不做进阶补全」，**不新建平行章节**，本日转为在既有页内补「跨框架横向对比」与「2026 版本状态」两节，深化成果计入存量配额 |
| 83 | Backend | 缓存设计 ✅（**改道**：转既有页深化） | 原定「缓存策略、穿透/击穿/雪崩、一致性」已由 `DB/NoRelational/Redis/Advanced/CacheDesign` 与 `CacheProtection` 两页完整覆盖（含三大问题防护），按防重复规则**不建平行章节**；本日转而在既有两页内补「多级缓存与一致性（本地 → Redis → DB）」与「缓存与业务一致性的三种取舍」 |
| 84 | Backend | 搜索引擎深入 ✅（**改道**：转既有页深化） | 原定「ES 调优、中文分词、索引生命周期」已由 `DB/NoRelational/Elasticsearch` 9 页覆盖（含 `ChineseAnalyzer` / `Cluster` / `Practice`），按防重复规则**不建平行章节**；本日转而在既有专题内补「索引生命周期管理（ILM 与 Data Stream）」与「向量检索与 ES 的衔接」两节 |
| 85 | DB | 分库分表 ✅ | 概述与决策树、拆分策略与分片键、分布式 ID、ShardingSphere 主线、跨分片查询与分布式事务、平滑迁移、实战（订单表分库分表）、常见问题（**库内空白**：全库 14 处零散提及但无独立主题，`ShardingSphere` **0 命中**；本日新建 `docs/DB/Relational/Sharding` 9 页，定位为「关系型数据库的水平扩展」，与 `MySQL/IndexPerformance` 的**单机优化**形成分层） |
| 86 | Ops | 云原生 | 云厂商、Serverless、容器服务、成本优化（**已核对**：容器编排层已由 `Ops/Kubernetes` 与 `Ops/ContainerOrchestration` 覆盖，下一天**不要以 K8s 为主体重讲**；库内「Serverless / 无服务器」仅 4 处提及、「成本优化」仅出现在日志与监控专题的成本段，**Serverless 与云成本治理（FinOps）是真空白**，第 86 天应聚焦此后半段） |
...
- [x] Frontend 存量补全：数据可视化专题已建设（第 81 天轮换主题）——`docs/Frontend/DataVisualization` 9 个页面（主题首页 + 概述与选型 + 渲染路线（Canvas / SVG / WebGL）+ ECharts 深入 + 数据到图形的映射 + 大数据量下的性能工程 + 数据大屏工程 + 实战：实时监控大屏 + 常见问题与最佳实践），共 2844 行。**定位**：与 [Vue3 · ECharts] 明确分工——那边讲「在框架里怎么封装图表」（hooks、类型、目录组织），本专题讲「可视化本身」（渲染路线怎么选、数据怎么映射成图形、规模上去了怎么办）。内容覆盖：探索式与展示式可视化的分界、按表达目标选几何图形（避免「用错视觉通道得出相反结论」）、渲染三条路线的本质差异与决策清单、ECharts 分层架构与 **option 合并语义**（默认合并 / `replaceMerge` / `notMerge` 三方式对照，以及「残留组件删不掉」的成因）、数据的三种给法（`series.data` / `dataset.source` / `dataset` + `transform`）、视觉通道可辨识度排序（位置 > 长度 > 角度 > 面积 > 颜色 > 形状）、比例尺与三类配色（分类 / 顺序 / 发散）、**LTTB 降采样完整实现**与 Web Worker / OffscreenCanvas、大数据量四层优化（关动画 / `appendData` / `progressive` / `sampling`）、大屏三种适配方案（含 contain 语义的 `createScaler`）与实时数据流（环形缓冲 + 指数退避 + 补数）、7×24 稳定性七项、1920×1080 实战骨架与服务端模拟器（含 90 秒尖刺）；含 9 张 SVG 示意图 + 1 张官方 Logo（ECharts PNG，75% 居中，只放主题首页），子页图片一律 `../assets/`；`frontend.ts` 新增 `FrontDataVisualization` 子树，`sidebar.ts` import 并加入 `/docs/Frontend` 汇总与主题级挂载，`nav.ts` 前端分组与 `docs/Frontend/index.md` 补入口

- [x] DB 存量补全：分库分表专题已建设（第 85 天轮换主题）——`docs/DB/Relational/Sharding` 9 个页面（主题首页 + 概述与决策 + 拆分策略与分片键设计 + 分布式 ID 生成 + ShardingSphere 主线 + 跨分片查询与分布式事务 + 平滑迁移 + 实战：订单系统分库分表 + 常见问题与排错）。**定位**：与 `MySQL/IndexDeepDive` 与 `SQLOptimization` 的**单机优化**形成分层——先做单机优化，装不下、写不进才轮到分片。内容覆盖：**「单表多大算大」的 B+ 树换算**（16KB 页 / 1170 指针 / 三层约 2190 万行，按行宽折算）、四种拆分方向与「永久失去的 8 项能力」清单、MySQL 分区表作为替代方案的完整 DDL 与 4 条限制、分片键四条硬要求与「与业务主键同源」的路由表方案、四种分片算法对照（取模 / 范围 / 一致性哈希 / 虚拟桶）与分片数定法（取 2 的幂 + 留 2~4 倍余量）、雪花算法完整实现（含时钟回拨的等待与抛异常两种处理）与号段模式双 buffer、ShardingSphere 五步内核（解析 / 路由 / 改写 / 执行 / 归并）与 Spring Boot 完整配置、**深分页的复合游标**（`(created_at, id) < (?, ?)`）、跨分片 JOIN 五种解法与聚合改写（`AVG` 为什么算错的算例）、分布式事务三方案与本地消息表完整实现、六步迁移法（双写开关 + 分批迁移 + 三层校验 + 灰度切读的量化判据）、实战含 `docker-compose.yml` 两个 MySQL 8.4、16 张分表生成脚本、路由枚举自检脚本 `check_routing.py` 与并发测试；含 9 张 SVG 示意图 + 1 张官方 Logo（PNG，75% 居中）；`db.ts` 新增 `Sharding` 子树（**定义位置必须在 `Relational` 之前**——`Relational` 的数组字面量里用展开语法引用了它，若定义在后会在模块加载时因暂时性死区抛 ReferenceError），`sidebar.ts` 三处挂载，`nav.ts` 与 `docs/DB/Relational/index.md` 补入口

- [x] 存量事实核对（第 81 天轮换配套）：**ECharts 与 ShardingSphere 的版本口径，以及一处高频误解**。① ECharts 主线为 **6.1.0（2026-05-19）**，6.0.0 发布于 2025-07-30（默认主题大改：图例默认移到底部、坐标系防溢出默认开启），5.6.0（2024-12-11）是 5.x 末版；专题内统一按「6.x 为主线 + 5.x 写法仍在用」标注。② Apache ShardingSphere 最新发布版为 **5.5.3（2026-02-28）**，5.5.4 处于开发中，5.4 及更早已停止维护；运行时支持 OpenJDK 26 与 GraalVM CE on JDK 25。③ **必须澄清的一处高频误解**：官网首页的 6.x 到 8.x 时间线是**项目路线图的篇章编号**（5.x 可插拔微内核、6.x To Cloud、7.x To Ecosystem、8.x 规划中），**不是已发布的版本号**——选型时应以 `Latest Releases` 页为准；这条已写入专题首页与 ShardingSphere 页的 `:::warning`。

- [x] 存量文档更新（第 81 天配套，15 篇，逐条写明分工边界）：`Frontend/Basic/Browser/Rendering`（补两条绘图支线的分工：本页讲渲染管线怎么走、可视化专题讲图表场景怎么选支线）、`Frontend/Basic/CSS/Advanced/Animation`（补「不走 CSS 的动画」——`requestAnimationFrame` 循环的三条纪律与 SVG 属性过渡为何每帧重算）、`Frontend/Others/PerformanceOptimization/Runtime`（补特例实战：十万点降采样与大屏内存泄漏排查）、`Frontend/index.md`（分类目录补两处入口）、`Ops/Monitoring/Grafana`（补业务大屏的自研路线与「与 BI 工具的分工」判据）、`Frontend/Frame/React`（补 React 侧画图表的通用原理、`useRef` / `useEffect` 封装与 Vue hooks 同构）、`Frontend/Frame/Vue/Vue3`（补「工程封装 vs 可视化本身」的分工）、`Frontend/Frame/Vue/Vue3/ECharts`（版本现状指向专题 + 相关文档三条）、`DB/Relational` 目录页（新增分库分表入口）、`DB/Relational/SQLOptimization` 目录页（新增「与分库分表的分工」段：单机优化 vs 水平扩展，判据是「能用索引解决的不要用分片解决」）、`DB/DataModeling` 目录页（补「分片键是一次不可逆的建模决策」）、`Backend/Microservices/DistributedTransaction`（补分工：本页讲原理与选型、分片页讲分片场景下怎么用）、`DB/NoRelational/MongoDB/Sharding`（补「托管分片 vs 应用/中间件分片」的对比）、`DB/NoRelational/Redis/Advanced/Cluster`（补「缓存侧可丢可重建、数据库侧是唯一副本」的取舍差异）、`Others/ProjectDelivery/DataModel`（补六步法在分片场景下的具体化，以及多出的「路由规则切换」这一不可逆节点）

- [x] 存量整理任务（第 81 天取一项）：**侧边栏「逆向覆盖」巡检（本次新立并结清）**。既有 `sidebarlink.py` 只查单向（侧边栏/导航里写的链接 → 文件是否存在），本次补齐反向的两个盲区并沉淀为 `sidebarcover.py`——**Check A（主题级覆盖）**：磁盘上每个「分类目录的直接子目录」或「已挂载主题的直接子目录」是否在侧边栏/导航里有入口；**Check B（目录页清单一致性）**：主题目录 `index.md` 列出的子页与实际子目录是否一一对应（报出「实际存在但未列」与「列出但磁盘上没有」）。首轮跑出 **14 错 + 28 告警**，逐条处置：**真实缺项 9 处已修**——其中最有价值的一处是 `LangChain4j` 整个主题的 **14 个页面从未挂载侧边栏**（已补进 `backend.ts` 的 Frame 子树并在 `Frame/index.md` 补入口），其余为 `Frame/Langchain4j` 漏列 QuickStart、`Frontend/Basic` 漏列 Browser、`SpringSecurity/v6` 的两个分组行由纯文本改为可点链接、`Basic/CSS` 漏列 CSS3 / Preprocessor / AtomicCSS 三个分组落点、`AI/OpenClaw` 漏列核心概念、`project/Base/Vue3Template` 漏列 Service、`AI/index.md` 漏列 Java 占位页；**工具误报 1 类已修**（正则把 `](./index.md)` 之类误判为子页，改为要求子目录名以字母/数字/下划线/短横线开头）；**分类核查 2 处已处置**——`SpringMVC/v6/index.md` 是 **0 字节空页**却被 `SpringBoot/v3/Web` 正文引用（点击即空白页），已把引用改为指向 `SpringMVC/index.md` 并移除空目录；`SpringCloud/` 下的 `2020` / `2022` / `SpringCloud2020` / `SpringCloud2022` 是迁移前的平铺草稿且文件名含空格，按「只登记不合并」在 `SpringCloud/index.md` 新增「历史版本存档」表并标注为仅存量。**同日意外收获**：`project/Base/Vue3Template/` 下有一个目录名首字符是 **U+200C 零宽非连接符**，肉眼与普通 `Stylelint` 完全无法区分、也无法手工输入，已改名并同步修正 `project.ts` 与目录页两处引用。终态：**Check A/B 错误 0、告警 32**（告警均为「仅靠父目录页列表可达、未直接挂载侧边栏」的正常设计，不是缺陷）。

- [x] Project 存量补全：当月项目（周期 3「后端通用模板」，第 81 天）添加第 13 个可验证构建步骤，作为**第 4 周④（部署与验收）的需求插入**——**主库可插拔**：主库不再固定 MySQL 8，改为 **MySQL 8.4 LTS / PostgreSQL 17 部署期二选一**。新增 [主库可插拔：MySQL / PostgreSQL 双方言] 模块页与 1 张 SVG 示意图 `database-pluggable.svg`，覆盖双方言建表脚本（`db/mysql/` + `db/postgres/`）、**结构一致性门禁** `parity_check.py`（类型归一化后逐列比对，避免两套 DDL 悄悄漂移）+ 11 项自测、双 profile 数据库编排（`docker-compose.databases.yml`）、分页与方言差异的显式注入；理由是「主库选择会影响验收清单的备份/恢复命令与监控指标口径，先定引擎再收口验收，避免同一件事走两遍」。**原第 81 天计划「上线验收与监控接入」顺延至第 82 天**（该页在 [进展记录] 中以 `:::warning` 显式记录了顺序调整与理由），第 4 周里程碑仍为 3/4。同步更新 [项目总览]（技术选型表「主库固定」改「主库可插拔」、进度表拆出第 81 天行、交付内容补第 20 项、本地运行补第 7 步）与 [进展记录]（新增第 81 天段落），`project.ts` 侧边栏补条目

- [x] 并行写入冲突的发现、备份与去重（第 81 天，本次新立并结清）：本日发现**同一仓库上有另一个并行会话在同时写入**，与本会话重复建了「数据可视化」专题——对方落在 `docs/Frontend/DataV`（9 页 / 1174 行 / 侧边栏已挂 `FrontDataV`），本会话落在 `docs/Frontend/DataVisualization`（9 页 / 2844 行）。**发现方式**：跑 `linkcheck.py` 时输出里出现一批本会话从未创建的 `DataV/*` 路径。**处置**：① 先整目录备份到仓库外（`~/.workbuddy/tmp/DataV_backup`）；② 确认对方 4 分钟内无新写入后删除重复目录；③ **保留并沿用**对方在项目侧的完整产出（`project/Base/BackendTemplate` 的第 81 天主库可插拔模块、`project.ts` 挂载、项目总览与进展记录更新），避免破坏已完成的项目侧配额；④ 清理对方挂载留下的三处残留引用（`sidebar.ts` 的 import 与汇总项、`nav.ts` 的分组项），改为指向本会话目录；⑤ 把对方写下的 **8 篇存量交叉链接**（`Browser/Rendering`、`CSS/Advanced/Animation`、`PerformanceOptimization/Runtime`、`Frontend/index.md`、`Ops/Monitoring/Grafana`、`Frame/React`、`Frame/Vue/Vue3`、`Vue3/ECharts`）重写到新路径并**逐条修正层级深度**——实测其中 4 处链接的 `../` 数量不足（如从 `Vue3/` 用两级 `../` 指 `docs/Frontend/DataV`，实际需三级），说明**并行写入不仅会重复，还会带进未经校验的链接**，这类断链在对方未跑巡检前不会暴露。**约定（本次新增）**：同一仓库不并行跑两个会改同一目录树的会话；若已经并行，合并方必须**重跑全库巡检**，而不是采信对方「构建通过」的结论。

- [x] 第 82 / 83 / 84 天改道深化已落地（第 81 天一并完成，成果计入存量配额）：三个原定主题经核查均与库内既有专题重叠，按 AGENTS.md 第 5 节「已有内容的主题不做进阶补全、不新建平行章节」转为在既有页内深化——① **状态管理**：在 [Pinia 进阶] 新增「跨框架横向对比」（Pinia / Redux Toolkit / Zustand / NgRx 六个维度对照 + 三条可套用判据：「状态该放哪」先于「用哪个库」、可变与不可变的差别在调试不在性能、异步写法决定代码形态）与「版本现状（2026-09 核对）」（四个库的迁移要点：Pinia 3.x 删旧插件签名、RTK 2.x 收紧 reducer 行为、Zustand 5.x 默认 `useSyncExternalStore` 并移除 `equalityFn`、NgRx 跟随 Angular 主版本），并给出「别照抄旧文版本号」的判据；② **缓存设计**：在 [缓存设计] 的「多级缓存」下新增「本地缓存的失效难题：三条路线」（广播失效 / 短 TTL 兜底 / 版本号轮换，含 Spring Data Redis 订阅完整代码）与「一致性与『先删缓存还是先写库』」，并给出「广播消息会丢、必须配短 TTL 兜底」「L1 永不过期就是永久脏数据」「L1 与 L2 的两级写顺序不能反」三条 `:::danger`；③ **缓存防护**：新增「缓存与业务一致性的三种取舍」（强一致 / 最终一致 / 以缓存为准三档对照表 + 各自适用业务），其中档位二给出**必须放在事务提交之后删缓存**的 `afterCommit` 完整写法与「事务内先删缓存、并发读回填旧值」的漏洞分析；④ **搜索引擎**：在 [索引与映射] 新增「索引生命周期管理（ILM 与 Data Stream）」（四个阶段与「年龄从 rollover 时刻算起」这一最易误解点、按 `max_primary_shard_size` 滚动的完整 policy、data stream 模板与 `_ilm/explain` 验证，以及「必须有 `@timestamp`」「不能删文档」「不能直接写后备索引」「不能对数据流名建索引」四条约束），在 [查询 DSL] 新增「向量检索与 ES 的衔接」（`dense_vector` 四个参数与各自易错点、kNN 查询与 `num_candidates` 约等于 `k` 乘 10 的召回判据、带 filter 的 kNN、**RRF 混合检索**「不做分数归一化、只看排名、免调参」的原理与完整请求体、ES 做向量库的五行定位对比表，以及「语义检索只是补充维度就用 ES、向量检索是产品全部才上专用库」的判据）
