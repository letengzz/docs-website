...
**前置条件**：接口文档必须生成为 swagger 抛出openapi 的接口 json

![image-20250329112107899](assets/image-20250329112107899.png)

## 配置请求统一函数 request.ts

安装对应依赖包和对应@types申明依赖包：

```shell
npm i axios lodash-es qs
npm i --save-dev @types/lodash-es @types/qs
```
...
@umijs/openapi 是 umijs提供的一套用于配合 openapi 生成接口的插件。[https://www.npmjs.com/package/@umijs/openapi](https://gitee.com/link?target=https%3A%2F%2Fwww.npmjs.com%2Fpackage%2F%40umijs%2Fopenapi)

- 依赖包安装：

  ```shell
  npm i @umijs/openapi --save-dev
  ```
