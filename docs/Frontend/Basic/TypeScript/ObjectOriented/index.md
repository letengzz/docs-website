...
```typescript [function-overload.ts]
...
![属性修饰符](../assets/img202408202105454.png)
...
class Student extends Person {
 ...
 console.log(`${this.age}岁的${this.name}正在努⼒努力学习`)
 }
}
```
...
```typescript [protected-inherit.ts]
class Student extends Person {
  constructor(name: string, age: number) {
    super(name, age)
  }
  study() {
    ...
    this.introduce()
    // ⼦类中可以访问name
    console.log(`${this.name}正在努⼒努力学习`)
  }
}
...
```typescript [object-interface.ts]
...
```typescript [multi-generic.ts]
function swap<T, U>(tuple: [T, U]): [U, T] {
  return [tuple[1], tuple[0]]
}
const result = swap<string, number>(['hello', 100])
console.log(result) // [100, 'hello']
```

泛型接口：

```typescript [generic-interface.ts]
interface GenericFn {
  <T>(arg: T): T
}
...
```typescript [generic-constraint.ts]
...
// loggingIdentity(100) // 报错：number没有length属性
```
