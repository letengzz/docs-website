...
fs(`file system`，文件系统)模块，是 Node.js 中的内置模块，可以对计算机中的磁盘进行操作，实现与硬盘的交互。例如：文件的创建、删除、重命名、移动还有文件内容的写入、读取以及文件夹的相关操作。
...
**说明**：使用 fs 模块的时候，尽量使用 `__dirname` 将路径转化为绝对路径，这样可以避免相对路径产生的 Bug
