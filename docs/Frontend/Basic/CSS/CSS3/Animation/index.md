...
一段动画，就是一段时间内连续播放 n 个画面。每一张画面，管它叫做"帧"。一定时间内连续快速播放若干个帧，就成了人眼中所看到的动画。同样时间内，播放的帧数越多，画面看起来越流畅。
...
animation-timing-function ，设置动画的类型，常用值：
...
animation-iteration-count ，指定动画的播放次数，常用值：

1. number ：动画循环次数

2. infinite ： 无限循环

animation-direction ，指定动画方向，常用值：
...
animation-play-state ，设置动画的播放状态，常用值：
...
```css
.inner {
  animation: hjc 3s 0.5s linear 2 alternate-reverse forwards;
}
```
