...
一个应用仅仅只有界面展示是不够的，还需要和用户做交互，例如：响应用户的点击、获取用户输入的值等等，在小程序里边，就通过编写 `JS` 脚本文件来处理用户的操作，但是微信小程序**没有 `DOM` 和 `BOM`**。
...
事件处理函数需要写到 `.js` 文件中，在 `.js` 文件中需要调用小程序提供的 `Page` 方法来注册小程序的页面，可以直接在 `Page` 方法中创建事件处理函数：
...
当组件触发事件时，绑定的事件的处理函数会收到一个事件对象，用来记录事件发生时的相关信息。在触发事件时，事件处理程序会主动的传入一个参数：`event` (事件对象)。
