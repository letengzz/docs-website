...
在<span v-pre>`{{ }}`</span>内部可以做一些简单的运算，支持如下几种方式：
...
在<span v-pre>`{{ }}`</span>语法中，只能写表达式，不能写语句，也不能调用 js 相关的方法。
...
7. 使用数组的`splice()`方法删除元素：

   ```js
   // 使用数组的 splice 方法来删除元素
   const newList = this.data.animalList.splice(1,1)
...
2. 尚不能写 data 路径，也就是不支持数组和对象，错误用法：
