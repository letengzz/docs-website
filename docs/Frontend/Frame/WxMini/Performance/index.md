...
当上述常规手段都已用尽，仍有两类「结构性」优化：

| 瓶颈类型 | 判断依据 | 手段 |
| --- | --- | --- |
| 渲染层瓶颈 | 节点多、动画掉帧、滚动卡顿 | 改用 [Skyline 渲染引擎](../Skyline/index.md)，或用 worklet 做渲染层动画 |
| 计算阻塞主线程 | 点击后卡住、白屏一段时间后恢复 | 把纯计算搬进 [Worker](../Worker/index.md) |

两者解决的是**不同层的问题**，不能互相替代：

- 卡顿来自「节点太多 / 渲染引擎能力上限」→ 换 Skyline 或做虚拟列表；
- 卡顿来自「一次重计算占满 JS 线程」→ 用 Worker。

::: warning 说明
这两项都属于**结构性改动**，成本明显高于路径更新、分包这类常规优化。建议顺序是：先把常规手段做到位并用真机数据确认瓶颈位置，再决定是否引入 Skyline 或 Worker。
:::
...
- [Skyline 渲染引擎](../Skyline/index.md)：突破渲染层性能天花板
- [Worker 多线程](../Worker/index.md)：把重计算搬离主线程
