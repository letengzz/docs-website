...
+     })
    ...
    wx.request({
      // 接口地址
      url: 'https://gmall-prod.cn/mall-api/index/findBanner',
      // 请求方式
      method: 'GET',
      // 请求参数
      data: {},
...
小程序中也能够像网页一样支持本地数据缓存，本地数据缓存是小程序存储在当前设备上硬盘上的数据，本地数据缓存有非常多的用途，可以利用本地数据缓存来存储用户在小程序上产生的操作，在用户关闭小程序重新打开时可以恢复之前的状态。还可以利用本地缓存一些服务端非实时的数据提高小程序获取数据的速度，在特定的场景下可以提高页面的渲染速度，减少用户的等待时间。其包含以下 8个主要的 API：
...
::: tip 深入阅读
本节只覆盖路由 API 的基础用法。页面栈上限、四种导航方式的取舍、返回刷新与传参设计见 [路由与页面栈](../Router/index.md)；版本相关的能力判断见 [基础库版本与兼容](../Version/index.md)。
:::
