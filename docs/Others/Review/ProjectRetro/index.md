...
- [完整项目交付](../../ProjectDelivery/index.md)：本页讲项目**结束后**怎么复盘，该专题讲项目**过程中**怎么交付（需求拆分 → 架构决策 → 契约先行 → 数据迁移 → 测试门禁 → 一键部署 → 上线验收）。两者互补而非重复：交付方法论把「下次别再犯」的结论**前置**成流程约束，复盘的产出应当回流进这套清单，否则复盘结论只停在文档里
