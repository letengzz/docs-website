...
::: warning 客户端改表结构有风险
Navicat 的表设计器让改表变得很容易，但**改结构前必须先想清楚是不是建模问题**：加字段能解决，还是应该拆表？大表上直接改类型/加索引可能触发长时间锁。模型层面的判断见[数据建模](../../../DB/DataModeling/index.md)，大表变更的注意事项见[实战：模型演进](../../../DB/DataModeling/Practice/index.md)。

另外建议：**结构变更一律留迁移脚本**，不要只在客户端点几下就上线——否则新环境建库与生产结构就对不上了。
:::
