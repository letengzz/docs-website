...
- [时序数据库](../TimeSeries/index.md)：当数据是"时间 + 标签 + 数值"时，建模规则完全不同（标签基数、序列爆炸、时间分区）
- [电商系统设计 · 领域建模](../../Backend/Ecommerce/DomainModeling/index.md)：把本页的通用建模原则（范式、主键策略、索引组合）放到**最有代表性的有状态交易系统**上——SPU/SKU 两层商品模型、价格六层计算与分摊、订单与库存的边界划分，以及"什么时候必须反范式"的具体判据
- [分库分表 · 拆分策略与分片键设计](../Relational/Sharding/Strategy/index.md)：分片键本质上是一次**不可逆的建模决策**——一旦按 `user_id` 拆开，所有不带 `user_id` 的查询都要付额外代价；本页的主键与命名规范是它的前置条件
