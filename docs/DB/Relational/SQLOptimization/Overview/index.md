...
::: warning 优化的上限在建模阶段就决定了
优化优先级里的第 3 条（表结构设计）意味着：**如果表结构本身不支持高效查询，索引怎么调都救不回来**。索引该建在哪些列、联合索引顺序怎么排、哪些字段是受控冗余，都应在[数据建模](../../../DataModeling/index.md)阶段确定（见[设计原则与规范](../../../DataModeling/DesignPrinciples/index.md)的「索引前置考虑」）。
:::
