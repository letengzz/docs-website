...
请求由客户端向后端服务器发起，默认的工作都是由一层层的过滤器完成的，经过层层过滤器，最后到达Controller。

在Spring中，将Filter 当做Bean对象来管理，在添加删除启用禁用，就会更灵活。

![filterchain](assets/img202402241522175.png)

### AbstractSecurityWebApplicationInitializer

`onStartup()`方法会在启动时被调用：
...
![delegatingfilterproxy](assets/img202402241522462.png)
...
通过断点调试，发现这里放回的对象是一个FilterChainProxy类型的，并且调用了它的初始化方法。
...
![filterchainproxy](assets/img202402241522345.png)

`DelegatingFilterProxy`执行doFilter方法：
...
![securityfilterchain](assets/img202402241522396.png)
...
![multi-securityfilterchain-17016804731631](assets/img202402241522529.png)
...
![image-20231204230216259](assets/img202402241522159.png)

### SecurityProperties

默认情况下Spring Security将初始的用户名和密码存在了SecurityProperties类中。这个类中有一个静态内部类User，配置了默认的用户名（name = "user"）和密码（password = uuid）

![image-20231205164049257](assets/img202402241522474.png)

可以将用户名、密码配置在SpringBoot的配置文件中：在application.properties中配置自定义用户名和密码
