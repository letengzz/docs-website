...
向量库的容量包含三部分，**漏算任何一项都可能在上线后 OOM**。

| 组成 | 估算方式（float32） | 100 万条 × 1536 维示例 |
| --- | --- | --- |
| 原始向量 | `维度 × 4 字节` | 1536 × 4 ≈ 6 KB/条 → **约 6.0 GB** |
| 图结构（HNSW） | 约 `M × 2 × 4~8` 字节/条 | M=16 → 约 1.3~2.6 GB |
| 元数据与原文 | 取决于字段数量与文本长度 | **常被低估**，直接存原文时往往超过向量本身 |

压缩手段与代价：

| 手段 | 存储变化 | 召回影响 |
| --- | --- | --- |
| 降维（1536 → 1024 / 512） | 线性下降 | 视数据分布而定，必须实测 |
| int8 量化 | 约 **1/4** | 通常损失很小，性价比最高 |
| 二值量化 | 约 **1/32** | 损失明显，需配合重排（先二值粗排、再用原始向量精排） |

::: danger 三个常被漏算的地方
1. **元数据比向量更占空间**：把 `text` 原文直接存在向量库里时，百万级 chunk 的文本体积通常远超向量本身。
2. **HNSW 的图不是零成本**：内存型方案要把图一并放进内存，`M` 调大时内存增长很明显。
3. **备份与副本成倍**：生产至少一主一从，容量要按 2 倍预留；再叠加索引重建期间的新旧两份，峰值可能到 3 倍。
:::

## 多模态向量的存与查

图文同库时，**索引结构与文本检索完全相同**，差别只在三处：

| 差异点 | 做法 |
| --- | --- |
| 向量来源 | 文本与图像分别由同一多模态模型的对应编码器产出，**维度与向量空间必须一致** |
| 类型标记 | 元数据加 `modality` 字段，检索时按需过滤（只要文本 / 只要图像 / 都要） |
| 索引重建 | 换多模态模型时必须**同时**重建文本侧与图像侧，只重建一边会让跨模态检索整体失效 |

评测口径与文本检索一致，但**评测集必须包含跨模态查询**（用图查文、用文查图）——只用单模态评测集，反映不出跨模态对齐的质量。

- 编码侧的模型选择与维度权衡：[嵌入与向量基础 · 多模态嵌入](../Embedding/index.md)
- 多模态链路的完整工程：[多模态应用 · 图像与文档理解](../../Multimodal/ImageUnderstanding/index.md)
