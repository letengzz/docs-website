# Maven

![img](https://maven.apache.org/images/maven-logo-black-on-white.png)

- [Maven 概述](Basis/Introduce/README.md)
- [Maven 安装及配置核心程序](Basis/Install/README.md)
- [Maven 核心概念](Core/README.md)
- [Maven 创建工程](Create/README.md)
- [Maven 命令](Command/README.md)
- [Maven 依赖](Dependency/README.md)
- [Maven 集成和聚合特性](Integrated/README.md)
- [Maven 拓展概念](Expansion/README.md)
- [Maven profile](profile/README.md)
- [Maven 私服：Nexus(暂)](Nexus/README.md)

**拓展**：

- [Maven jar包冲突](jar/README.md)
- [IDEA 配置及使用 Maven](../../IDE/IDEA/Maven/README.md)
- [JavaWeb 单一架构案例](Single/README.md)
- [SSM 整合伪分布式案例](SSM/README.md)
- [构建微服务架构](Microservice/README.md)