...
当 `DOCS_PLAN.md` 中的主题全部完成（均标记 ✅）后，协作方式切换为「**以项目为中心持续沉淀**」，规则如下：

1. **每次都要在 `project/` 中添加项目**：完成计划后的每一次任务，都必须对 `project/` 目录产生实际增量，不允许只做技术文档的零散补丁。
2. **项目数量宁少勿多，质量宁精勿滥**：`project/` 中同时在维护的项目**建议保持在 3~6 个**。新增项目必须能被长期迭代，禁止为了凑数创建"一周后就无法继续写"的空壳项目。
3. **每个项目都要「大而全」**：一个项目从初始化、环境、规范、核心模块、数据、权限、测试、构建、部署、监控到复盘，要形成闭环，读者照着做能跑起来。
4. **项目必须逐步完成、细节到位**：不要求一次写完全部章节；按模块逐次推进，每次把当前模块写到"能照着做一遍"的颗粒度（完整的文件内容、完整的命令、可验证的收尾）。
5. **优先在既有项目中新增模块**：新一轮迭代默认以「给现有项目加模块/加章节」的方式作为本次更新内容（例如新增权限模块、主题模块、部署模块、监控模块），章节内更新一律在既有页面调整，不新建重复章节。
6. **每轮迭代要留下可验证的痕迹**：
   - 更新项目目录页 `index.md`（新增章节必须挂进目录页与侧边栏）；
   - 在项目目录页的「迭代记录」中写明本次新增/修改了哪些模块；
   - 运行 `pnpm docs:build` 验证；
   - 按 Conventional Commits 提交（`docs(project): ...`）并 `git push origin main`。
7. **项目文档同时受第 9 节约束**：完整文件内容用 `[文件名]` 代码块标题、每个操作给完整命令与验证方式、截图与示意图放各自章节的 `assets/`。
8. **技术文档与项目文档的关系**：`docs/` 负责讲原理与用法，`project/` 负责把它们串成可交付的工程；项目页要用相对链接回指 `docs/` 中的对应技术专题，避免重复讲解。
