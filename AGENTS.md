...
8. 每个技术主题的**首页（`index.md`）必须放置该技术的官方 Logo**：Logo 使用 **PNG 格式**，下载到主题 `assets/`（如 `assets/nginx-logo.png`），并用既有缩放写法 `<img src="./assets/xxx-logo.png" style="zoom:75%;" />` 统一缩放到 **75%**（注意 `./` 前缀，否则 VitePress 构建无法解析）；标注图片来源，不要用随意截图代替官方 Logo。
