...
4. **已有内容的主题不做「进阶补全」**：不因「已有章节过浅」而新建整套重复章节（如已有 Basic 又新建 BasicAdvanced）；需要补充、加深的内容直接在**既有章节页面内更新调整**，合并重复内容后统一维护。只有内容确实超出原主题范围（新的大版本、新的生态方向）时，才考虑新增独立章节，且必须先确认库内没有对应页面，避免重复。
...
- 主题目录的 `index.md` 是**侧边栏入口**：侧边栏根节点 `text` 用主题名，`link` 指向该 `index.md`，`items` 是该主题的子页面，**顶层根节点默认展开（不写 `collapsed`）**，其下所有子分组显式设置 `collapsed: true`。
...
6. 每个主题在 `sidebar.ts` 中按**主题路径**挂载（如 `"/docs/Backend/Java": Java`）：进入 `docs/Backend/Java` 才展示 Java 的侧边栏，根节点是 `Java/index.md`；**顶层主题节点（如 Java、Python）默认展开，不写 `collapsed`**，其下所有子分组显式设置 `collapsed: true`。
...
4. 图片放入对应页面目录的 `assets/`，使用相对路径引用；主题首页（`index.md`）必须放置该技术官方 Logo，正文按主题配图。
...
- **已有内容的主题不做「进阶补全」**：存量章节的加深与补充一律在既有页面上更新调整，禁止为同一主题新建重复章节；发现重复内容时先合并再维护。
