...
单档压测只能回答"这个并发下扛不扛得住"，回答不了"再加一倍会怎样"。**容量拐点**问的是后一个问题：持续加并发，加到某一档 TPS 不再跟着涨（甚至回落）、同时 P95 开始陡升，那一点就是这套配置的容量上限。

找到它的方法是**阶梯加压**，而不是一次压到底：

| 档位 | VU | 持续时间 | 关注量 | 记录什么 |
| --- | --- | --- | --- | --- |
| 1 | 10 | 2m | 基准 | TPS、P95、CPU、连接池占用 |
| 2 | 25 | 2m | 线性段 | 每加 15 VU 换来多少 TPS（增量） |
| 3 | 50 | 2m | 逼近段 | 增量是否已明显衰减 |
| 4 | 100 | 2m | 拐点段 | TPS 是否不再上升、P95 是否陡升 |
| 5 | 200 | 2m | 过载段 | 是否出现错误率 > 1% 或超时 |

判定拐点用**两条证据同时成立**，任一条单独出现都不算：

1. **TPS 增量衰减**：`(TPSₙ − TPSₙ₋₁) / (VUₙ − VUₙ₋₁) < 前一段增量的 50%`；
2. **资源水位触顶**：CPU 饱和（> 85%）、连接池排队（`SHOW FULL PROCESSLIST` 有 Waiting）、或 GC 时间占比 > 10%。

只满足第 1 条可能是**测试机自己成了瓶颈**（k6 与 SUT 抢 CPU）；只满足第 2 条可能只是压测脚本里 `sleep` 设太小导致的无效并发。两条同时成立，才是"服务端容量上限"。

结果记录表（与第 9 节基线表一起提交，因为拐点离开环境描述就没有意义）：

| 档位 | VU | TPS | TPS 增量/VU | P95 | CPU | 连接池峰值 | 结论 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | 10 | 待填 | — | 待填 | 待填 | 待填 | ⏳ |
| 2 | 25 | 待填 | 待填 | 待填 | 待填 | 待填 | ⏳ |
| 3 | 50 | 待填 | 待填 | 待填 | 待填 | 待填 | ⏳ |
| 4 | 100 | 待填 | 待填 | 待填 | 待填 | 待填 | 待填 ← **拐点在这里吗** |
| 5 | 200 | 待填 | 待填 | 待填 | 待填 | 待填 | ⏳ |

找到拐点之后要做的三件事，顺序不能颠倒：

1. **把拐点写进容量台账**，并据此定告警阈值（例如取拐点 TPS 的 70% 作为扩容线）。没有这一步，拐点只是一次实验数据。
2. **检查是哪个资源先触顶**，只对那一个扩容：CPU 触顶加副本有效，连接池触顶要先查是否漏了分页/慢查询，内存触顶要先看是否有无上限的本地缓存。
3. **重跑一次第 4 档**确认可复现。单次观测到拐点，也可能是当时机器上跑了别的东西。

:::tip 提示
验收清单里的 **B2「容量拐点有记录」** 判据就是本页存在这一节且写明资源水位与并发点：`python Acceptance/acceptance_check.py` 会检查本页同时出现"容量"与"拐点"两处记录，并人工确认表已填。拐点是**能力边界**，不是缺陷——把边界记下来，才不用在事故当天现场摸。
:::
