...
15. [异常路径联调收口与用例清单](./ErrorPath/index.md)：**入口级异常与业务异常是两套体系**——发生在进 Controller 之前（JSON 反序列化失败、请求体超限、路由无匹配）的异常走独立一组处理器，混进 `BizException` 会让人误以为已覆盖。请求体超限**两层都要拦**（`ContentLengthGuardFilter` 按 `Content-Length` 在读报文前拒绝 + Tomcat 的 `max-http-form-post-size` / `max-swallow-size`），只靠一层会分别出现「响应结构不一致」与「连接被占住、偶发连接池耗尽且日志无痕」。并发更新冲突用 **409 而不是 400**（请求合法、冲突在资源状态）；错误信息给到**字段名 + 合法取值列表**；响应体**绝不回显原始报文**（可能含密码、身份证号等敏感字段），排障信息只进带 traceId 的服务端日志。另含「接口 × 场景」用例清单（9 接口 × 10 场景）与守住用例数基线的**元测试**、`AsyncUncaughtExceptionHandler` 补齐请求线程之外的盲区、三层测试隔离档位的团队约定。

16. [进展记录](./Progress/index.md)：逐日做了什么、如何验证、下一步是什么。
